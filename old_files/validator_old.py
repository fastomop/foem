...
class SqlTest:
    ...
    def patients_2drugs_and_time(self):
        """
        Return a parameterised SQL + params tuple to count distinct patients
        taking two drug *families* (all descendants of each mapped standard concept)
        whose exposure start dates are within `days` of one another.
        """
        ...
        for d1, d2 in result:
            text = f"""
            Counts of patients taking drug {d1} and {d2} within 30 days.
            """
            ...
            query, params = self.templates.patients_2drugs_and_time(v_id1, v_id2, d_id1, d_id2, 30)
    ...
    def patients_2drugs_and(self):
        ...
        for d1, d2 in result:
            text = f"""
            Counts of patients taking drug {d1} and {d2}.
            """
            v_id1, d_id1 = self.templates.find_code_by_name(d1, self.vocab_dict)
            v_id2, d_id2 = self.templates.find_code_by_name(d2, self.vocab_dict)
            query, params = self.templates.patients_2drugs_and(v_id1, v_id2, d_id1, d_id2)
            query = self.__finalise_sql(query, params, self.conn)
            print(text, "\n", query, "\n\n\n\n\n\n")
